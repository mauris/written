...
\begin{document}
  ...
  \paragraph{License} This reference is made publicly available under the MIT License. You should not pay anyone money in exchange for this document. The source code for this document can be found at \href{https://github.com/mauris/written}{https://github.com/mauris/written}. If you wish to help improve this document, feel free to open an issue on the Github repository.
 ...
 Returns $true$ if $Element$ does not occur in $List$, $false$ otherwise.
  ...
  Returns $true$ if $List1$ is a permutation of $List2$. As both $List1$ and $List2$ are expected to not be a variable, this method cannot be used to generate permutations of a given list. $permutation/2$ can be defined as follows:
