...
\item $F\phi$ encode situations in which "$\phi$ will become \textbf{true} at some point in future." Since $F$ is the dual of $G$, $F\phi ::= \lnot G\lnot\phi$.
...
\begin{comment}
TODO: Add the entry that G and F can be constructed from U
\end{comment}
...
\subsubsection{Satisfaction on Paths}
...
\item $EF\phi \equiv \phi \lor EXEG\phi$
