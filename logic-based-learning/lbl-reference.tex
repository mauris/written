...
\paragraph{} NOTICE: This set of reference is currently incomplete and will be updated as the semester progress. By Mar 2018 it should be complete.
...
\paragraph{} A Normal Logic Program is set of rules, where each rule $R$ is in the form
...
\paragraph{} For a definite logic program $P$, a Herbrand Interpretation $I$ of $P$ is an Herbrand Model if for all rule $R$ st
...
\paragraph{} We can construct the least Herbrand model for a logic program $P$, denoted $M(P)$ by starting with the empty set $M = {}$. We then repeatedly add any atom $h$ to $M$M st $h$ is the head of a rule $R$ whose body is a subset of $M$, i.e. we add $head(R)$ to $M$ if $(body^+(R) \cup body^-(R)) \subseteq M$. For definite programs with no loops, this is the same as what is provable using Prolog.
